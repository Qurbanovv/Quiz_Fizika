...
function loadQuestion(index) {
  ...
  if (selectedAnswer) {
    [btn_A, btn_B, btn_C, btn_D, btn_E].forEach((btn) => {
      btn.disabled = false;
...
function partialShuffleRange(array, start, end) {
...
document.getElementById("shuffle_range").addEventListener("click", () => {
  const start = parseInt(document.getElementById("mix_start").value) - 1;
  const end = parseInt(document.getElementById("mix_end").value) - 1;
  ...
  partialShuffleRange(questions, start, end);
  answersGiven = new Array(questions.length).fill(null);
  loadQuestion(start); // qarışıq hissənin başlanğıcına gedək
});
